...
int NvEncoder::FramesInQueue()
{
	return m_iToSend - m_iGot;
}
...
void NvEncoder::EncodeFrame(std::vector<NvEncOutputFrame> &vFrames, NV_ENC_PIC_PARAMS *pPicParams)
{
	vFrames.clear();
    ...
    DoEncode(m_vMappedInputBuffers[i], vFrames, pPicParams);
}
...
void NvEncoder::DoEncode(NV_ENC_INPUT_PTR inputBuffer, std::vector<NvEncOutputFrame> &vFrames, NV_ENC_PIC_PARAMS *pPicParams)
{
	...
	if(picParams.pictureStruct <=0 || picParams.pictureStruct >3)
		picParams.pictureStruct = NV_ENC_PIC_STRUCT_FRAME; //need to set it
    ...
    {
        m_iToSend++;
        GetEncodedPacket(m_vBitstreamOutputBuffer, vFrames, true);
    }
    else
    {
...
void NvEncoder::EndEncode(std::vector<NvEncOutputFrame> &vFrames)
{
	vFrames.clear();
    if (!IsHWEncoderInitialized())
    {
    ...
    GetEncodedPacket(m_vBitstreamOutputBuffer, vFrames, false);
}

void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, std::vector<NvEncOutputFrame> &vFrames, bool bOutputDelay)
{
    unsigned i = 0;
    ...
    for (; m_iGot < iEnd; m_iGot++)
    {
        ...
        if (vFrames.size() < i + 1)
        {
			vFrames.emplace_back();
        }
		vFrames[i].encodedData.clear();
		vFrames[i].encodedData.insert(vFrames[i].encodedData.end(), &pData[0], &pData[lockBitstreamData.bitstreamSizeInBytes]);
		vFrames[i].timeStamp = lockBitstreamData.outputTimeStamp;
		vFrames[i].frameDuration = lockBitstreamData.outputDuration;
		vFrames[i].isKeyFrame = (lockBitstreamData.pictureType & (NV_ENC_PIC_TYPE_I | NV_ENC_PIC_TYPE_IDR | NV_ENC_PIC_TYPE_BI)) > 0;
        i++;
