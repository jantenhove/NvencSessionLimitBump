...
inline bool check(CUresult e, int iLine, const char *szFile) {
    if (e != CUDA_SUCCESS) {
        ...
        //LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("CUDA driver API error " << szErrName << " at line " << iLine << " in file " << szFile));
        return false;
    }
    return true;
}
...
inline bool check(cudaError_t e, int iLine, const char *szFile) {
    if (e != cudaSuccess) {
        LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("CUDA runtime API error " << cudaGetErrorName(e) << " at line " << iLine << " in file " << szFile));
        return false;
    }
    return true;
}
...
inline bool check(NVENCSTATUS e, int iLine, const char *szFile) {
    ...
    if (e != NV_ENC_SUCCESS) {
		//LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("NVENC error " << aszErrName[e] << " at line " << iLine << " in file " << szFile));
        return false;
    }
    return true;
}
...
inline bool check(HRESULT e, int iLine, const char *szFile) {
    if (e != S_OK) {
       // LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("HRESULT error 0x" << (void *)e << " at line " << iLine << " in file " << szFile));
        return false;
    }
    return true;
}
...
inline bool check(GLenum e, int iLine, const char *szFile) {
    if (e != 0) {
        LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("GLenum error " << e << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(int e, int iLine, const char *szFile) {
    if (e < 0) {
        //LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("General error " << e << " at line " << iLine << " in file " << szFile));
        return false;
    }
    return true;
}
    ...
    BufferedFileReader(const char *szFileName, bool bPartial = false) {
        ...
        while (nSize) {
            try {
                ...
                if (nSize != st.st_size) {
                    //LOG4CPLUS_WARN(logger, LOG4CPLUS_TEXT("File is too large - only " << std::setprecision(4) << 100.0 * nSize / (uint32_t)st.st_size << "% is loaded"));
                }
                break;
            } catch(std::bad_alloc) {
                if (!bPartial) {
                    //LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("Failed to allocate memory in BufferedReader"));
                    return;
                }
        ...
        if (!fpIn)
        {
            //LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("Unable to open input file: " << szFileName));
            return;
        }
